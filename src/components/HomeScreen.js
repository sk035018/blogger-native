...
const HomeScreen = () => {
  ...
  const onDelete = async id => {
    try {
      const {
        data: { errMsg },
      } = (await deleteBlog(id, stateToken)) || { data: {} };
  ...
  const renderData = ({ item }) => (
    ...
    >
      <VStack p="2">
        <Text textAlign="center" fontSize="24" w="100%" bold underline>
          {item.title}
        </Text>
        {item.subTitle && (
          <Text textAlign="center" fontSize="16" w="100%" italic>
            {item.subTitle}
          </Text>
        )}
        ...
        <HStack justifyContent="space-between" mt="5" mb="1">
          ...
          <Button px="2" py="1" size="sm" onPress={() => onDelete(item._id)}>
            Delete
          </Button>
        </HStack>
      </VStack>
